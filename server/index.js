...
// DB restore PEHLE (async), phir db.js require karo taaki khaali DB par
// purana backup wapas aa jaye. Isliye db/routes ke require boot() ke andar hain.
const { restoreFromCloudinary, startAutoBackup } = require('./src/dbBackup');
...
// ---- boot --------------------------------------------------------------------------
async function boot() {
  // 1. Agar local DB khaali hai to Cloudinary se purana backup wapas lao.
  await restoreFromCloudinary();

  // 2. Ab DB kholo (pehli baar nahi — restore ke baad).
  ({ db } = require('./src/db')); // opens DB + creates schema on require
